...
For that, the `start.S` was updated to jump to the C code for the CPU = 0, given by the `cpu_0` label and `b entry` instruction. `entry` is the label which correspondent to the `entry` function in the `main.c` file (when compiling the code, the C functions become labels that can be referenced by the Assembly code). The `entry` is responsible for initializing the UART and writing some characters to it.
You can check the characters by wiring some cables to the UART ports in the Pi and using a USB<->FTDI adapter between this wiring and your PC USB port. Then, you can read from the UART in your PC with some serial program, like `minicom`. For example, as the code is using baud rate = 115200, you can run: `sudo minicom -D /dev/ttyACM0 115200 -c off`. Use `sudo dmesg` to find the port connected by the USB<->FTDI adapter as it might no be `/dev/ttyACM0`, but something like `/dev/ttyUSB0`.
